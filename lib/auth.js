    ...
    .then(function(existingUser) {
      if (existingUser) {
        ...
        req.user.save({
          ...
          gender: req.user.get('gender') || profile._json.gender,
          picture: req.user.get('picture') || profile._json.picture
        })
        .then(function (model) {
          Tokens.forge({
            user_id: model.get('id'),
      ...
      .then(function(existingEmailUser) {
        if (existingEmailUser) {
          ...
          user.save({
            ...
            tokens: user.get('tokens').push({kind: 'google', accessToken: accessToken}),
            name: user.get('name') || profile.displayName,
            gender: user.get('gender') || profile._json.gender,
            picture: user.get('picture') || profile._json.picture
          })
          .then(function (model) {
            Tokens.forge({
              user_id: model.get('id'),
