...
module.exports.setup = function (hbs) {
  ...
  hbs.registerHelper('gravatar', function(user, size, context) {
    size = size || 32;
    ...
    let md5 = crypto.createHash('md5').update(user.email);
  ...
  hbs.registerHelper('isFeatured', function(flag, id, context) {
    let answer = parseInt(flag, 10);
    let link = '';
  ...
  hbs.registerHelper('lastMessageDate', function(messages, user, context) {
    let len = messages.length - 1;
    return parseDate(messages[len].created_at, 'HH:mm MMM D, YYYY');
  });


  hbs.registerHelper('printMessage', function(conversation, user, context) {

    let unread = conversation.messages.filter(function(msg) {
      return msg.read === 0 && msg.from_id !== user.id;
    });
    let lastMsg = conversation.messages[0];
    let lastMsgDate = parseDate(lastMsg.created_at, 'HH:mm MMM D, YYYY');
    let lastMsgBody = lastMsg.body.substring(0, 160) + '...';;

    let aLink = `<a href="/admin/messages/${conversation.id}" class="list-group-item">`;

    let unreadSpan = unread.length ? `<span class="badge">${unread.length}</span>` : '';

    aLink += unreadSpan;

    let nameHeader = `<h4 class="list-group-item-heading">`;

    if(user.id === conversation.from.id) {
      let src = gravatar(conversation.to.email, 24);

      nameHeader += `<img src="${src}" class="img-circle"> ${conversation.to.name} <small>${lastMsgDate}</small>`;
    }
    else {
      let src = gravatar(conversation.from.email, 24);

      nameHeader += `<img src="${src}" class="img-circle"> ${conversation.from.name} <small>${lastMsgDate}</small>`;
    }
